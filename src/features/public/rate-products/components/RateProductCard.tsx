...
import { ChevronDown, MapPin, Instagram , Facebook, User, Star } from "lucide-react"
...
export function RateProductCard({ product }: ProductCardProps) {
  ...
  useEffect(() => {
    const votedProducts = JSON.parse(localStorage.getItem("votedBurgers") || "{}")
      ...
      <div className="cursor-pointer">
        <div className=" relative overflow-hidden">
          <LazyLoadImage
            ...
            className="object-cover hover:scale-105 transition-transform duration-300 h-96 w-full "
          />
          <div className="absolute top-4 right-4">
            <Badge variant="secondary" className="bg-white/90 text-gray-800">
              {product.restaurant.locations.length} sede{product.restaurant.locations.length !== 1 ? "s" : ""}
            </Badge>
          </div>
          {alreadyVoted && (
            <div className="absolute top-4 left-4">
              <Badge className="bg-green-600 hover:bg-green-600 text-white">Ya votaste</Badge>
            </div>
          )}
        </div>
          ...
          <div className="space-y-4">
            {/* <div className="flex items-center justify-between">
              ...
              <div className="space-y-4 animate-in slide-in-from-top-2">
                ...
                <div className="">
                  <h4 className="font-semibold text-gray-900 mb-3">Información del Restaurante</h4>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 text-sm mb-2">
                        ...
                        <span className="text-muted-foreground font-medium">
                          Sede{product.restaurant.locations.length !== 1 ? "s" : ""}:
                        </span>
                      </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2 text-sm mb-2">
                        <MapPin className="w-4 h-4 text-muted-foreground" />
                        <span className="text-muted-foreground font-medium">
                          Direccion{product.restaurant.addresses.length !== 1 ? "es" : ""}:
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-900 font-medium">{product.restaurant.owner}</span>
                    </div>
                  </div>

                  <div className="flex gap-2 mt-4 relative bottom-0">
                    <Button variant="outline" size="sm" className="flex items-center gap-2 h-8 bg-transparent">
                      <Instagram className="w-3 h-3" />
                      <span className="text-xs">{product.restaurant.socialMedia.instagram}</span>
                    </Button>
                    <Button variant="outline" size="sm" className="flex items-center gap-2 h-8 bg-transparent">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      {alreadyVoted ? (
                        <div className="text-center">
                          <h4 className="font-semibold text-gray-900 mb-3">Tu calificación</h4>
                          <div className="flex flex-col items-center gap-3">
                            <StarRating rating={userRating} readonly={true} size="lg" showLabels={true} />
                            <Badge variant="default" className="bg-green-600 hover:bg-green-600">
                              Ya calificaste esta hamburguesa
                            </Badge>
                            <p className="text-xs text-muted-foreground">
                              Diste {userRating} estrella{userRating !== 1 ? "s" : ""} a este producto
                            </p>
                          </div>
                        </div>
                      ) : (
                        <div>
                          ...
                          <div className="flex flex-col items-center gap-4">
                            <StarRating rating={userRating} onRate={handleRate} size="lg" showLabels={true} />
                            {userRating > 0 && !hasVoted && (
                              <Button onClick={confirmVote} className="bg-red-600 hover:bg-red-700" size="sm">
                            ...
                            {hasVoted && (
                              <div className="text-center">
                                ...
                                <p className="text-xs text-muted-foreground mt-2">
                                  Tu voto de {userRating} estrella{userRating !== 1 ? "s" : ""} ha sido registrado
                                </p>
                              </div>
                            )}
                          </div>
